...
public static class UpdateService
{
    ...
    private const string RepoName = "CollectorUI";
    ...
    {
        s_http.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("CollectorUI", GetCurrentVersionString()));
        s_http.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/vnd.github+json"));
    }
    ...
    {
        var basePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "CollectorUI", "updates");
    ...
    public static async Task<string> DownloadAssetAsync(string url, IProgress<double>? progress = null, CancellationToken ct = default)
    {
        ...
        using var resp = await s_http.GetAsync(url, HttpCompletionOption.ResponseHeadersRead, ct);
        resp.EnsureSuccessStatusCode();
